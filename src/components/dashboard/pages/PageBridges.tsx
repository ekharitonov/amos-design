  ...
  return (
    <div className="animate-fade-in">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
        <h2 className="font-display text-[22px] font-bold">Изнанка: Слой III — Мосты</h2>
      </div>

      <div className="bg-surface border border-border rounded-2xl p-6 lg:p-8">
        <h3 className="font-display text-lg mb-5">Карта Связей и Мостов</h3>
        ...
        <div className="relative">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
            ...
            <div className={`transition-all duration-300 ${detail ? "opacity-100 translate-y-0" : "opacity-40 translate-y-2"}`}>
              {detail ? (
                <div className="bg-card/80 backdrop-blur-sm rounded-xl border border-border p-6 sticky top-20">
                  <div className="flex justify-between items-start mb-5 gap-3">
                    <div>
                      <span className="text-xs text-muted-foreground font-mono-brand tracking-wider uppercase block mb-1">Мост</span>
                      <span className="text-[17px] font-bold">
                        {detail.personA} <span className="text-primary">↔</span> {detail.personB}
                      </span>
                    </div>
                    <span className={`text-[11px] px-3 py-1.5 rounded-md font-semibold border whitespace-nowrap ${tensionColor(detail.tensionLevel)}`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    {detail.gives.map(s => (
                      <div key={s.who}>
                        <div className="text-[13px] font-semibold text-primary mb-3">{s.who}</div>
                        {s.items.map(it => (
                          <div key={it} className="text-[13px] text-text-dim py-2 border-b border-border/60 flex items-start gap-2 leading-snug">
                  ...
                  <div className="mt-5 p-3 bg-background/50 rounded-lg border border-border/50">
